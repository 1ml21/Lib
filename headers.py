from ZAminofix.lib.util import signature,sign_data
import random
from uuid import uuid4
from PyPanorama import UserAgentGenerator
...
class LoginHeaders:
	def __init__(self, data = None, deviceId: str = None):
		headers = {
			"NDCDEVICEID": device_id,
			"Content-Length": str(len(data)),
			"NDC-MSG-SIG": str(signature(data)),
            "NDCLANG": "en",
...
class ApisHeaders:
    def __init__(self, data = None, type = None, deviceId: str = None, sig: str = None):
        ...
        if data:
            headers["NDC-MESSAGE-SIGNATURE"] = sign_data(data, userId, sid, device_id)
